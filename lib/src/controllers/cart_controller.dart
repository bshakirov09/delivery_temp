...
class CartController extends ControllerMVC {
  ...
  void goCheckout(BuildContext context) {
    if (!currentUser.value.profileCompleted()) {
      ScaffoldMessenger.of(scaffoldKey?.currentContext).showSnackBar(SnackBar(
        content: (currentUser.value.profileAddress())
            ? Text(S.of(state.context).completeYourProfileVeriToContinue)
            : (currentUser.value.profilePhone())
                ? Text(S.of(state.context).completeYourProfilePhoneToContinue)
                : (currentUser.value.profileVerifiedPhone()) ?
          Text(S.of(state.context).completeYourProfileVeriToContinue) :
        Text(S.of(state.context).completeYourProfileDetailsToContinue),
